...
function PlanController($scope, plannerSrv, messageSrv) {
    $scope.maxDepth = 30;
    $scope.needAdvancedCert = function () {
        return plannerSrv.isDeep($scope.maxDepth);
    };
    $scope.message = function () {
        return messageSrv.certMessage(plannerSrv.isDeep($scope.maxDepth))
    };
}